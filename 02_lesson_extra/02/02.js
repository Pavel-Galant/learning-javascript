...
function revertTicket(ticket, nowTime) {
    const flight = ticket.split('-')[0]; // всегда возаращает 1 элемент
    if (!(flight in flights)) throw new Error('Not found flight!');
    
    // ищем билет в данном рейсе
    const index = flights[flight].tickets
        .findIndex(item => (item.id === ticket));
    // если нашли, то пробуем дальше отменить билет   
    if (index > -1) { 
        // если меньше 2 часов до окончания региcтрации (от окончания регистрации до начала рейса 1 час) 
        // не смотрим такой рейс, т.к. отменить билет можно за 3 часа до начала полета
        if (!(flights[flight].registartionEnds - nowTime > 2*3600000)) return false; 
        const result = flights[flight].tickets[index].type !== 1; // можно отменить если не бизнес-класс
        if (result) flights[flight].tickets.splice(index, 1); //удалить с переиндексацией
        return result;   
    }
    throw new Error('Not found ticket!');
}

console.log(revertTicket('BH118-B50', makeTime(12, 5)));
